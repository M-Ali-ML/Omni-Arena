...
export default function ArenaPage() {
  ...
  // Only worth labelling rows when there is more than one scale in play.
  const splitGraph = (leaderboard.components.count ?? 1) > 1;
  ...
  return (
    <main>
      ...
      <form className="prompt-form" onSubmit={submitPrompt}>
        ...
        {arena.isStreaming && (
          <button type="button" onClick={() => void arena.stop()}>
            Stop both models
          </button>
        )}
          ...
          {leaderboard.models.map((model) => (
            <li key={model.id}>
              <span className="model-name">
                {model.displayName}
                {splitGraph && model.componentId !== null && (
                  <span className="component-tag">
                    group {model.componentId}
                  </span>
                )}
              </span>
