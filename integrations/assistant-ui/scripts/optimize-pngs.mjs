...
//
// Chromium writes 24-bit PNGs, and a 2560x1600 retina shot of a mostly-flat UI
// is about a megabyte of that. Re-encoding as a quantised palette PNG keeps the
// text crisp — the app uses few distinct colours — at a fraction of the bytes.
// Runs as the last step of `npm run screenshots`, or on its own after replacing
// a single image.
...
const outputDir = fileURLToPath(
  new URL("../../../docs/images/integrations/assistant-ui", import.meta.url),
);
...
try {
  ...
  console.warn(
    "sharp is not installed, leaving the PNGs unoptimised. " +
      "Run `npm install` in integrations/assistant-ui.",
  );
  process.exit(0);
}
