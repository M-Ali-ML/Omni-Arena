  ...
  const handler = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (plan.kind === "shadow") {
      ...
      try {
        await dependencies.repository.createMatchup({
          ...
          slotAModelId: incumbent.id,
          slotBModelId: challenger.id,
          slotAProviderModelId: incumbent.providerModelId,
          slotBProviderModelId: challenger.providerModelId,
    ...
    try {
      await dependencies.repository.createMatchup({
        ...
        slotAModelId: assignment.slotA.id,
        slotBModelId: assignment.slotB.id,
        slotAProviderModelId: assignment.slotA.providerModelId,
        slotBProviderModelId: assignment.slotB.providerModelId,
        matchupTokenHash: issuedToken.hash,
        harnessVersion: dependencies.harnessVersion,
        mode: "blind",
        conversation: {
